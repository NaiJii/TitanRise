...
struct Matrix3x4_t {
	...
	void SetColumn(const Vector_t& value, int column) noexcept;
	void AddToColumn(const Vector_t& value, int column) noexcept;
	...
	Matrix3x4_t& BuildRotationAboutAxis(const Vector_t& axis, float degrees) noexcept;
	Matrix3x4_t Invert() noexcept;
	Angle_t ToAngle() const noexcept;
	Vector_t Transform(const Vector_t& value) const noexcept;
